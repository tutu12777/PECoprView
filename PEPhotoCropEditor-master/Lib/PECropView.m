...
{
    NSLog(@"croprect===%@====%@", @(cropRect.origin.x), @(cropRect.origin.y));
    self.cropRectView.frame = cropRect;
//    self.cropRectView.center = CGPointMake(cropRect.size.width / 2, cropRect.size.height / 2);
...
- (void)setupImageView
{
    ...
    self.zoomingView.backgroundColor = [UIColor redColor]; [UIColor clearColor];
...
- (void)cropRectViewDidEndEditing:(PECropRectView *)cropRectView
{
    self.resizing = NO;
    [self zoomToCropRect:self.cropRectView.frame];
//    CGFloat width = CGRectGetWidth(cropRectView.frame);
//    CGFloat height = CGRectGetHeight(cropRectView.frame);
//    NSLog(@"cropRectView===%@====%@", @(width), @(height));
//    NSLog(@"contentOffset====%@====%@", @((CGRectGetWidth(self.bounds) - width) / 2), @( -(CGRectGetHeight(self.bounds) - height) / 2));
//    self.scrollView.contentOffset = CGPointMake((CGRectGetWidth(self.bounds) - width) / 2, (CGRectGetHeight(self.bounds) - height) / 2);
////    self.scrollView.contentOffset.y = height;
//    self.cropRectView.center = self.center;
////    [self layoutOverlayViewsWithCropRect:self.cropRectView.frame];
////    [self resetCropRect];
}
...
- (void)zoomToCropRect:(CGRect)toRect andCenter:(BOOL)center
{
    ...
    CGFloat scale = 1;//MIN(CGRectGetWidth(self.editingRect) / width, CGRectGetHeight(self.editingRect) / height);//1;
    ...
    NSLog(@"self.scro===%@", @(self.scrollView.zoomScale));
